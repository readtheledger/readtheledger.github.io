...
Imperium Post's own articles live in `content.js`, not in `index.html`: each entry carries its `kind` (`"news"`, `"analysis"` or `"deep"`), section, headline, body and a `sources` array crediting every piece of research behind it. Follow `REVIEW.md` before publishing an addition or substantive revision. Mark the current deep-dive feature with `weekly:true`. Each entry may carry an `image` — its own picture, with alt text, size, caption, credit, an `ai` flag and the derivative widths; `media.js` documents the contract and `docs/design/README.md` the policy (full 3:2 compositions, no crops, the caption adds no claim).

Every entry must explicitly set `produced`: `"reported"` (a person wrote it), `"assisted"` (a model drafted it from credited sources and a person checked every claim before publication), or `"ai-source-reviewed"` (AI drafted and source-checked it, with no human factual review claimed). The first two require the human factual-source and read-aloud checklist; the third requires the recorded two-AI claim/source review in `REVIEW.md`. The publication pull request records the actual review route and final hash. The eight original August 17 articles instead carry `"legacy-unrecorded"`: their retained evidence does not establish their production history or a recorded factual review. `production.js` limits that category to their pinned IDs and original dates and shares the attribution with the build and reader. Omitted, unsupported or new-article legacy values fail the build. Validation checks metadata, not whether a person or AI system really performed a review. The archive status also accompanies Listen and Copy. `drafts/` is where new work waits for actual review. `index.html` holds `WIM_NOTES`, the standing background notes by desk.

The additional `"ai-codex-checked"` designation records source checking by the drafting Codex task and a final prose review by a second Codex task. It states that human factual review is not documented for this version without denying other reported checks; it does not satisfy or alter the stronger condition for `"ai-source-reviewed"`. Follow the separate designation and evidence requirements in `REVIEW.md`.
...
python3 qa.py            # 53 checks: layout, touch targets, copy, listen fallback,
                         ...
                         # editorial mix, Imperium Post sourcing and the licence model
